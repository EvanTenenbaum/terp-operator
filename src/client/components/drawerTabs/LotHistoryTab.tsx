...
/**
 * LotHistoryTab — command journal for the active lot/batch.
 *
 * CAP-011 / TER-1486: Shows the commands that touched the selected batch
 * (intake post, flag, reject, reverse, sale reservations, …) so the
 * operator can trace the audit trail without leaving the intake/inventory
 * workspace. Pairs with `LotMovementTab` (which shows qty deltas).
 *
 * Uses `queries.relatedCommands({ entityId })`, which reads
 * `command_journal.affected_ids @> [batchId]` via the GIN index
 * (migration 0043).
 */

interface LotHistoryTabProps {
...
export function LotHistoryTab({ batchId }: LotHistoryTabProps) {
  const enabled = Boolean(batchId);
  const commands = trpc.queries.relatedCommands.useQuery(
    { entityId: batchId ?? undefined, contactId: undefined },
    { enabled }
  );
  ...
  if (!enabled) {
    return (
      <div className="context-drawer-card">
        ...
        <div className="drawer-empty mt-3">Select a batch to view its command history.</div>
      </div>
    );
  }
  ...
  return (
    <div className="context-drawer-card">
      ...
      {commands.isLoading ? (
        ...
        <div className="drawer-empty mt-4">No commands recorded for this batch yet.</div>
      )}
    </div>
  );
}
