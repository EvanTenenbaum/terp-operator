/**
 * TER-1671 final: Verify staging flicker fix — MutationObserver + click interactivity.
 *
 * The fix memoizes `rows` in InventoryFinderPanel.tsx and removes the broken
 * `areGridPropsEqual` comparator from OperatorGrid.tsx.
 *
 * Measures structural DOM mutations on the /sales page for 5 seconds.
 * Pass threshold: < 100 structural mutations/sec (previously was 1,628-1,859/sec).
 * Also verifies page is responsive by performing a click interaction.
 *
 * Run:
 *   PLAYWRIGHT_SKIP_WEB_SERVER=1 \
 *   PLAYWRIGHT_BASE_URL="https://terp-agro-staging-5asc2.ondigitalocean.app" \
 *     npx playwright test tests/e2e/final-flicker.spec.ts --project=chromium --workers=1
 */
...
// ── MutationObserver: structural-only count ─────────────────
...
test('TER-1671 final: mutation rate < 100/sec + click interactivity', async ({ page }) => {
  ...
  // Step 4: Screenshot before observation
  await page.screenshot({ path: 'test-results/final-flicker-pre-observe.png', fullPage: true });
  ...
  console.log(`  elapsed:       ${(counts.elapsedMs / 1000).toFixed(1)}s`);
  ...
  try {
    // Navigate back to a fresh /sales for reliable click test
    await page.goto('/sales');
    await page.waitForTimeout(2_000);
  ...
  await page.screenshot({ path: 'test-results/final-flicker-verdict.png', fullPage: true });
  console.log('\nScreenshot saved: test-results/final-flicker-verdict.png');
  ...
  // Step 9: Verdict
  const mutationPassed = counts.ratePerSec < MUTATION_THRESHOLD_PER_SEC;
  console.log(`\n  Threshold:     < ${MUTATION_THRESHOLD_PER_SEC}/sec`);
  console.log(`  Mutation rate: ${counts.ratePerSec}/sec → ${mutationPassed ? '✅ PASS' : '🔴 FAIL'}`);
  console.log(`  Click check:   ${clickSucceeded ? '✅ PASS' : '🔴 FAIL'}`);
  console.log(`  Overall:       ${mutationPassed && clickSucceeded ? '✅ PASS' : '🔴 FAIL'}`);
