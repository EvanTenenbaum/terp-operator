...
- Preferred checkout: `/Users/evantenenbaum/work/terp-operator`
- Valid linked worktrees or legacy local paths are acceptable when the git remote resolves to `EvanTenenbaum/terp-operator`
- If the remote is any other TERP-like repo, STOP and confirm
...
### Command Registry (ADR 0002) — MANDATORY pattern

**All new backend commands MUST use `defineCommand()` — NEVER add a `case` to the switch in `commandBus.ts`.**

Each command is one file at `src/domains/<domain>/commandDefs/<name>.ts` with `defineCommand({ name, input (Zod schema), rbac: { minimumRole }, reversal?, handler })`.
Handler signature: `(ctx, payload)` where `ctx = { tx, user, commandId, reason }`.

After adding: `pnpm typecheck && pnpm exec vitest run src/tests/commandRegistry.fitness.test.ts`.
