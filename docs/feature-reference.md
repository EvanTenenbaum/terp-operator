...
_Primary sources: `src/shared/types.ts`, `src/shared/commandCatalog.ts`, `src/server/db/schema.ts`, `docs/agent-orientation/domain-concepts.md`, `docs/workflow-gap-audit.md`_
...
A **Command** is the only way to mutate application state in TERP Operator. There are 130 named commands in `src/shared/commandCatalog.ts` (the `CommandName` union type). Commands are not REST endpoints — they are typed, named, idempotent operations executed through a centralized command bus.
...
User roles are defined in `src/server/db/schema.ts` as the `role` column on `users`:
...
6. Manager/owner runs `approvePurchaseOrder` → status moves to `approved`. This is the trigger that enables intake.
7. Operator runs `receivePurchaseOrder` → creates draft intake rows (batches with status `draft`) in the Intake view, one per PO line.
8. As intake progresses, PO status may reflect `partially_received`.
9. When all lines are received: status moves to `received`.
10. Cancellation: `cancelPurchaseOrder` → status moves to `cancelled`. Only possible if no lines have been received.
11. Prepayments can be recorded at any point after approval: `recordVendorPrepayment`.
...
5. Pricing applied: `priceSalesOrder` applies a pricing strategy. Line status moves toward `priced`. Sales order status may show `priced`.
...
- The `priced` status is an intermediate state — it does not block cancellation.
...
4. Orders remain visible here until status reaches `fulfilled` or `shipped`.
...
- Status `shipped` is a terminal state indicating the order left the warehouse.
...
- `pickLists.status` progresses: `open` → `in_progress` → `has_alerts` → `ready_to_close` → `closed` → `fulfilled`.
- Discrepancies between `expectedQty` and `actualQty` trigger the `has_alerts` state.
...
2. If `allocationIntent` is `fifo`: the system _intends_ to auto-allocate to oldest open invoices. **Note: see Known Gap J05 — auto-allocation may not execute, requiring manual step.**
...
**Key nuances:**
- **Known Gap (J05/DYN-H3):** `logPayment` with `allocationIntent='fifo'` does not auto-allocate. The operator must manually run `allocatePayment` after logging the payment. This is tracked as issue #26.
...
1. **Auto-creation:** `postPurchaseReceipt` (in Intake) automatically creates a `vendorBill` for non-consignment batches.
2. **Manual creation:** `createVendorBill` → creates a bill with status `created`.
3. `approveVendorBill` → status: `created` → `approved`.
...
- **vendorBills.status** values: `'open'`, `'created'`, `'approved'`, `'scheduled'`, `'paid'`, `'voided'`, `'partial'`, `'reversed'`.
...
| `createVendorBill` | Manually creates a vendor bill. |
| `approveVendorBill` | Approves a vendor bill. Status: `created` → `approved`. |
| `scheduleVendorPayment` | Schedules a vendor bill for payment. Status: `approved` → `scheduled`. |
| `recordVendorPayment` | Records payment against a scheduled bill. Status → `paid` or `partial`. |
| `voidVendorPayment` | Voids a vendor payment record. |
| `recordVendorPrepayment` | Records a prepayment against an approved PO. |
...
3. Manager can: set a new manual limit (`setCustomerCreditLimit`), revert to engine (`revertCustomerCreditToEngine`), adjust engine max (`setCustomerEngineMax`), change stance (`setCustomerStance`), or snooze the reminder (`snoozeCustomerCreditReminder`).
4. Global credit engine configuration: `setCreditEngineConfig` updates engine-wide parameters.
5. Stance management: `createCreditEngineStance`, `updateCreditEngineStance`, `deleteCreditEngineStance` manage the signal weighting profiles.
6. Bulk action: `bulkRevertCustomersToEngine` reverts all eligible manual-limit customers at once.
...
**Key nuances:**
- **Known Gap (J/DYN-H4):** `customer_needs` and `vendor_supply` status transitions are unconstrained today (issue #27). There is no enforced lifecycle — status can be set to any value without validation.
...
3. Operator reviews the request. Three outcomes:
   ...
   - `routeConnectorRequest` → routes to an operator queue (backend/internal behavior; not typically a direct user action).
...
- `routeConnectorRequest` is retained as backend/internal behavior. It should not be exposed as a direct operator UI action.
...
| `result` | jsonb | Command execution result data |
...
**Fix shipped:** Added invoice selector, bucket, reference, and notes fields to the payment UI. Added allocation to selected invoice or FIFO as UI options.

**Remaining concern (issue #26):** The auto-allocation execution path for FIFO intent may still require a manual `allocatePayment` step in some cases. Verify in the `logPayment` handler before assuming auto-allocation works end-to-end.

**Status:** Partially closed — verify FIFO auto-allocation execution.
...
**What was broken:** `customer_needs` and `vendor_supply` status transitions have no enforcement. Status can be set to any value without validation.

**Expected behavior:** Defined lifecycle with enforced transitions (e.g., `open` → `outreach_sent` → `matched` → `closed`).

**Fix shipped:** Not yet fixed as of this document.

**Status:** Open — status transitions remain unconstrained.
...
**Customer Need** — A record of a customer's product request, used in the Matchmaking workflow. Has no enforced lifecycle (known gap DYN-H4).
...
**Sales Order** — A customer order. Lifecycle: `draft` → `priced` → `confirmed` → `posted` → `fulfilled` → `closed`. Side exits: `cancelled`, `reversed`, `shipped`.
...
**Vendor Bill** — Money owed to a vendor. Created automatically on intake receipt posting. Lifecycle: `created` → `approved` → `scheduled` → `paid`. Side exits: `voided`, `partial`, `reversed`.

**Vendor Supply** — A record of a vendor's available product, used in the Matchmaking workflow. Has no enforced lifecycle (known gap DYN-H4).
