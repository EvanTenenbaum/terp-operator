...
| CAP-030 Pricing Rules Chain Manager | Operator request 2026-05-22 | Sell | control, context | Keep | R4, R12, R15 | Pricing rules are flat JSONB per-customer and in systemSettings; no consolidated view; no multi-condition rules. Spec at docs/superpowers/specs/2026-05-22-pricing-rules-chain-manager-design.md. **ID CANONICAL: CAP-030 = Pricing Rules only. See CAP-043 for the pick/fulfillment release-for-picking lineage (historical collision resolved 2026-06-12, UX-A14).** | Implement: pricing_rule_entries table, savePricingRuleChain command, PricingRulesView consolidated Settings tab, PricingRuleChainEditor component, resolvePricingRuleClause resolver. Migration from legacy JSONB with parity check and feature flag rollback. |
| CAP-043 Release-for-Picking (Warehouse Fulfillment) | decisions-log 2026-06-11 Decision 7 (A8); TER-1485/TER-1498/TER-1508/TER-1518/TER-1563 | Fulfill | core_workflow | Keep | R4, R6, R12 | **New CAP row assigned 2026-06-12 (UX-A14) to resolve historical CAP-030 collision.** Pick/fulfillment system ships under TER-1485/TER-1498/TER-1508/TER-1518/TER-1563. Commands: `releaseSelectedLines` (bulk release sales order lines to warehouse pick queue), `createPickList`, `allocateOrderToFulfillment`. Surfaces: PickView (`/pick`, CAP-030 lineage per TER-1563 nav comment — now corrected to CAP-043), pick queue socket events (sockets.ts:98), warehouse pick queue queries (queries.ts:1739), FulfillmentView. **Historical collision note:** source files (commandBus.ts:4339, queries.ts:1739, sockets.ts:98, Shell.tsx nav comment) carry `// CAP-030` comments referencing TER-1485/1498/1508/1518/1563. Those in-code comments are NOT being renamed in this run (out of scope per UX-A14 execution decision — registry + pointer note only). When a future agent refactors those comments, use CAP-043. | Update in-code `// CAP-030` comments in commandBus.ts, queries.ts, sockets.ts, and Shell.tsx to `// CAP-043` when the next relevant wave touches those files. |
...
| CAP-033 Banner dismiss | DB audit 2026-05-25 | All loops | control | Keep | R4 | `user_dismissed_banners` table exists with `(user_id, banner_key, dismissed_at)`; nothing is wired to it. `ShadowModeBanner` dismisses to localStorage only. Linear: TER-1587. | Build `banners.getDismissedBanners` tRPC query and `banners.dismissBanner` mutation; add `bannerKeys.ts` constants; wire `ShadowModeBanner` to DB-persisted dismissal on mount and on click. |
| CAP-034 Vendor → brand identity model | DB audit 2026-05-25 | Buy, Receive | infrastructure | Keep | R4, R12 | `brands` table exists but has no `vendor_id` column; `vendors` has no `primary_brand_id`. `createVendor` does not create a brand. `createBatch` never sets `brand_id` — always null. Brands filter facet returns nothing. Linear: TER-1585. | Add `vendor_id` to `brands` and `primary_brand_id` to `vendors`; write backfill migration with alias-collision handling; update `createVendor` to atomically create a brand in the same transaction; update `createBatch` and `updateBatch` to auto-resolve `brand_id` from `vendor.primary_brand_id`; update `realisticSeed.ts` to seed brand rows alongside vendors. |
| CAP-035 Secondary brands per vendor | DB audit 2026-05-25 | Buy, Receive | control | Keep | R4, R12 | Data model from CAP-034 supports multiple brands per vendor (`brands.vendor_id` + `vendors.primary_brand_id`). No commands or UI exist for adding secondary brands. Linear: TER-1589. Blocked by CAP-034. | Add `createSecondaryBrand`, `updateBrand`, `deactivateBrand` commands; surface in vendor detail panel as brand list with add/rename/deactivate controls; update batch intake to allow brand selection when vendor has multiple brands. |
| CAP-036 Sales Manager Dashboard | 2026-05-25 audit | Decide | projection | Needs Assessment | none | No implementation. | Assess with Evan. Phase 6 must land first. Scope: pipeline view, rep performance metrics, aged inventory alerts, commission visibility for managers. |
| CAP-037 Customer Intelligence Platform | 2026-05-25 audit | Sell, Decide | context, projection | Needs Assessment | none | No implementation. Partial: workspace.data.invoices/payments fetched in customerWorkspace query but not rendered. | Assess with Evan. CAP-001 covers point-of-sale start; CAP-037 is persistent customer intelligence across time: purchase affinity, cadence, churn risk, profile enrichment. |
| CAP-038 Quick Order Mode | 2026-05-25 audit | Sell | core_workflow | Needs Assessment | none | No implementation. recentCustomerSheets[0] and addAll logic exist in RecentSheetsPanel as a partial precursor. | Assess with Evan. Distinct from CAP-001 (New Sale start): a fast-path entry point pre-filled from order history for repeat customers. |
| CAP-039 Broker/Referee Management Module | 2026-05-25 audit | Sell, Buy, Collect/Pay | core_workflow, control | Needs Assessment | none | Partial: referee/refereeRelationships/refereeCredits tables and commands exist. RefereesView and RefereeDetailPanel exist. Missing: bulk pay accrued, performance report, inline credit prompt during sales, deactivated relationship history, credit ledger totals. | Assess with Evan. Extends existing referee system toward a full broker CRM. Not reducible to CMD-CONNECTOR. |
| CAP-040 Offline Pick Mode | 2026-05-25 audit | Fulfill | core_workflow | Needs Assessment | none | No implementation. PickView and pick flow exist as online-only. | Assess with Evan. Local-first pick list with sync-on-reconnect. Requires service worker, IndexedDB action queue, conflict resolution strategy. Separate capability class from CMD-FULFILLMENT (online). |
| CAP-041 Driver Manifest and Proof-of-Delivery | 2026-05-25 audit | Fulfill | projection, control | Needs Assessment | none | Partial: writeBagManifest called on recordWeighAndPack and markOrderFulfilled. pick_lists.tracking field exists. No driver-facing view. | Assess with Evan. Post-pack delivery tracking and manifest generation. Token-auth pattern proven by MediaUploadMobile. Distinct from CMD-FULFILLMENT (packing). |
| CAP-042 Barcode Scanning — Continuous Loop | 2026-05-25 audit | Receive, Fulfill | core_workflow | Needs Assessment | none | Partial: one-shot BarcodeDetector in PickLineScreen (Chrome/Android only). | Assess with Evan. Continuous-scan UX, multi-format support, iOS polyfill (react-zxing), hardware peripheral integration. Spans CMD-INTAKE (receive) and CMD-FULFILLMENT (pack). New capability layer. |

## Backend Command Families

| ID | Commands | Work loop | Exposure | Product decision | Next product move |
| --- | --- | --- | --- | --- | --- |
| CMD-INTAKE | `createBatch`, `updateBatch`, `deleteBatch`, `postPurchaseReceipt`, `adjustBatchQuantity`, `setInventoryStatus`, `transferInventoryLocation`, `transferInventoryOwnership`, `setBatchPrice`, `setBatchLotInfo`, `attachBatchPhoto`, `importBatchesCsv` | Receive | core_workflow, control | Already covered | Intake and Inventory surfaces expose draft edits, quantity/price/lot/media edits, validate-first CSV import, purchase receipt posting, and selected-batch status/location/ownership movements. `createBatch` will auto-resolve `brand_id` from `vendor.primary_brand_id` once CAP-034 lands (TER-1585). |
...
| CMD-BRANDS | `createSecondaryBrand`, `updateBrand`, `deactivateBrand` | Buy, Receive | control | Keep — blocked by CAP-034 | Commands for managing secondary brands on a vendor. Primary brand is auto-created by `createVendor` (CAP-034); these commands manage additional brands for multi-brand vendors. Linear: TER-1589. |
...
| BE-013 Media retention lifecycle | DB audit 2026-05-25 | Infrastructure | infrastructure | Keep | `media_retention_policies` and `media_cleanup_log` tables exist with no code touching them. No default policies seeded. `batchMedia` rows accumulate indefinitely. Linear: TER-1590. | Seed a default retention policy row; build a cron cleanup job that reads active policies, deletes/archives expired `batch_media` rows, and writes audit rows to `media_cleanup_log`. |
| BE-014 Contact merge-candidate detection | DB audit 2026-05-25 | Support | infrastructure | Keep | `contact_merge_candidates` table exists and is queried for a badge count in `queries.ts`, but no code ever populates it. Badge is always zero. Linear: TER-1591. | Build a scheduled detection job that scans `contacts` for similar names/emails/phones and inserts candidate pairs into `contact_merge_candidates`. Confirm the merge review UI surface location and wire it to show candidates. |
