...
| 14 | [Contacts and Appointments](#journey-14-contacts-and-appointments) | operator, manager | 2 |
...
- Path: `flagBatch` → appends an entry to the batch's `validationIssues` array and merges a note onto the parent PO's `internalNotes`. **Does NOT change `batches.status`.** There is no `'flagged'` status value in the system — the batch stays in its current status (usually `'draft'`). The validation issue is visible in the intake grid and must be resolved before the batch is marked ready and posted.
- Note: There is no `verifyBatch` command. The only bulk-verify path is `verifyAllIntake`.
...
- Path: `recordWeighAndPack` accepts any `actualQty > 0` — system does not block discrepancies at pack time. The discrepancy surfaces through `fulfillmentLines.warehouseAlerts` (jsonb). `recordWeighAndPack` sets `fulfillmentLines.status = 'packed'` regardless of qty match — **`'has_alerts'` is not a valid pick list status** (no DB write moves the pick list to that value). Manager reviews the warehouse alert and `acknowledgeWarehouseAlert`. Discrepancy is documented in the fulfillment line record, not on the pick list status.
...
2. If `allocationIntent = 'fifo'` or `'selected_invoice'`: system executes `allocatePayment` in the **same transaction** as `logPayment` (commandBus.ts:3697-3731). If no open invoices exist, auto-allocation is skipped gracefully and a toast notification informs the operator — the payment is logged as unapplied. Prior gap DYN-H3 / issue #26 is **closed**.
...
- `manager` — approves bills, schedules payments, can pass `overrideUnscheduled: true` on `recordVendorPayment` (the override is not separately role-gated)
- `owner` — same access as `manager` for AP operations
...
- **No additional role gate.** The override is NOT role-gated server-side — any role that has access to `recordVendorPayment` (minimum `manager`) can pass `overrideUnscheduled: true`. There is no separate `owner`-only check on the override flag.
...
1. Sales operator creates a customer need: `createCustomerNeed` → `customerNeeds` row with status `'open'`. Required: `customerId`, `productName`, `category`, minimum qty. Status transitions are enforced server-side via `assertValidNeedStatusTransition`: valid moves are `open → matched | closed`.
2. Sales operator creates vendor supply: `createVendorSupply` → `vendorSupply` row with status `'open'`. Required: `vendorId`, `productName`, `category`, available qty. Status transitions are enforced via `assertValidSupplyStatusTransition`: valid moves are `open → held_for_match | closed`.
...
| ⛔ Invalid need status transition | `updateCustomerNeed` attempting an invalid status move | `assertValidNeedStatusTransition` (commandBus.ts:5197-5224). Valid need transitions: `open → matched \| closed`. | Update |
| ⛔ Invalid supply status transition | `updateVendorSupply` attempting an invalid status move | `assertValidSupplyStatusTransition`. Valid supply transitions: `open → held_for_match \| closed`. | Update |
...
1. External connector submits a request via the API. Request lands in the `connectorRequests` queue with status `'open'` (the schema default — see `schema.ts:481`, `default('open')`). **No ledger mutations have occurred.**
...
5. Published: `publishBatchMedia` → `batchMedia.status` → `'published'`. `publishedAt` timestamp set.
   - **Note (implementation gap):** `publishBatchMedia` does **NOT** automatically update `batches.mediaStatus`. The modern upload+publish flow leaves `batches.mediaStatus` as `'open'` until a separate `updateBatch` call (or the legacy `attachBatchPhoto` command, which still does update `mediaStatus`) flips it to `'done'`. Operators using the modern flow must remember to update `mediaStatus` separately.
...
**Actors:**
- `operator` (sales) — creates contacts, manages appointments
- `manager` — archives contacts, links contacts to users and existing entities

**Entry condition:** A new person or business needs to be tracked. Or an existing contact needs an appointment scheduled.

**Exit condition:** Contact record exists with correct roles and linked entities. Appointments are scheduled, completed, or cancelled.

### Happy Path (Contact Creation)
1. Operator runs `createContact` → contact created with `contactKind` (`'individual'` or `'business'`), name, email, phone. No roles assigned yet.
2. Operator assigns role: `addContactRole` → links contact to customer, vendor, referee, or processor entity. Can assign multiple roles.
3. If contact should link to existing entity (migrated records): `linkContactToExistingEntity`.
4. If contact should link to a user account: `linkContactToUser` (requires `owner` role).
5. Operator updates details: `updateContact`.
6. Operator archives when inactive: `archiveContact`.

### Happy Path (Appointments)
1. Operator creates appointment: `createAppointment` → `appointments` row with status `'scheduled'`.
2. Appointment time or details change: `updateAppointment`. Guard: "Only scheduled appointments can be updated."
3. Appointment cancelled: `cancelAppointment` → status `'scheduled'` → `'cancelled'`. Guard: "Cannot cancel a completed appointment."
4. Appointment happens: `completeAppointment` → status `'scheduled'` → `'completed'`. Guard: "Cannot complete a cancelled appointment."

### Branch Scenarios
🔀 **B1 — Archive blocked by open obligations**
- Path: `archiveContact` checks for: open/partial invoices (customer), unpaid bills (vendor), active referee relationships, uncollected processor fees, outstanding balance. Throws with a specific message for each blocker.

🔀 **B2 — Deduplication (planned, not built)**
- The `contact_merge_candidates` table exists and deduplication detection is partially implemented. The merge UI (the operator-facing step to merge two contacts) is not yet built.

### Error States
| Error | Trigger | Message |
|-------|---------|---------|
| ⛔ Archive with open invoices | `archiveContact` when customer has open invoices | "Cannot archive: customer has open or partially-paid invoices." |
| ⛔ Archive with unpaid bills | `archiveContact` when vendor has unpaid bills | "Cannot archive: vendor has unpaid bills." |
| ⛔ Archive with active referee rels | `archiveContact` when referee is active | "Cannot archive: referee has active relationships." |
| ⛔ Already archived | `archiveContact` on archived contact | "Contact is already archived." |
| ⛔ Already linked | `linkContactToExistingEntity` on linked entity | "This customer/vendor/referee/processor is already linked to a contact." |
| ⛔ Update non-scheduled | `updateAppointment` on completed/cancelled | "Only scheduled appointments can be updated." |
| ⛔ Cancel completed | `cancelAppointment` on completed | "Cannot cancel a completed appointment." |
| ⛔ Complete cancelled | `completeAppointment` on cancelled | "Cannot complete a cancelled appointment." |

**Commands involved:**
| Command | Description |
|---------|-------------|
| `createContact` | Creates a universal contact record. |
| `updateContact` | Updates contact details. |
| `archiveContact` | Deactivates contact (with open-obligation guards). |
| `addContactRole` | Assigns a role (customer, vendor, referee) to a contact. |
| `linkContactToExistingEntity` | Links contact to an existing customer/vendor entity. |
| `linkContactToUser` | Links contact to a system user account (owner only). |
| `createAppointment` | Creates a scheduled appointment. |
| `updateAppointment` | Updates a scheduled appointment. |
| `cancelAppointment` | Cancels an appointment. |
| `completeAppointment` | Marks an appointment completed. |

**Entities touched:** `contacts`, `appointments`, `contactLedgerEntries`, `contactMergeCandidates`, `customers`, `vendors`.

**Connected features:** Foundation for Clients and Vendors (roles applied to contacts). Linked to Referees. Linked to Matchmaking (customer/vendor identity).

**Status:** ✅ Complete (backend). 🔄 Partial (frontend — contacts commands are in `pendingFrontendCommandNames`; UI partially wired).
...
|--------|-------|
| Journeys covered | 14 |
| Total branch scenarios | 86+ |
| Total error states (server-side guards) | 60+ |
| Total client-side gates documented | 18 |
| Cross-journey dependencies | 15 |
| Commands referenced | 130 |
| Exact error messages captured from code | 56+ |

> Note: Server-side guards total ~323 in `commandBus.ts` alone. The error states documented here cover the most operator-relevant subset, not the complete guard inventory.


---

## Completion Status and Milestone Map

> **Release train status (as of 2026-06-01):** The `codex/roadmap-execution-2026-05-12` release train is **complete**. 16/16 Playwright tests pass (Chromium). All four tranches — shell/canvas, backend kernel, operator surfaces, QA gate — closed with evidence.

### Journey Completion Status

| # | Journey | Status | Phase | Open Items |
|---|---------|--------|-------|------------|
| 1 | Purchase Order | ✅ Complete | Phase 2 | — |
| 2 | Intake / Receiving | ✅ Complete | Phase 2 | — |
| 3 | Inventory Management | ✅ Complete | Phase 2 | — |
| 4 | Sales Order | ✅ Complete | Phase 1 | — |
| 5 | Fulfillment | ✅ Complete | Phase 4 | — |
| 6 | Payments / AR | ✅ Complete | Phase 3 | Auto-allocation implemented; degrades gracefully when no open invoices (J05/DYN-H3 closed) |
| 7 | Vendor Bills / AP | ✅ Complete | Phase 3 | — |
| 8 | Closeout | ✅ Complete | Phase 5 | — |
| 9 | Recovery | ✅ Complete | Phase 5 | — |
| 10 | Matchmaking | ✅ Complete | Phase 2+ | Status lifecycle now enforced server-side via `assertValidNeedStatusTransition` / `assertValidSupplyStatusTransition` (DYN-H4 / issue #27 closed) |
| 11 | Connectors / Processors | ✅ Complete | Phase 4 | — |
| 12 | Credit Review | ✅ Complete | Phase 3–4 | Shadow mode surface — verify in `CreditReviewView.tsx` |
| 13 | Photography / Media | ✅ Complete | Phase 2 | `publishBatchMedia` does not update `batches.mediaStatus` (modern flow gap) |
| 14 | Contacts and Appointments | 🔄 Partial | Phase 2+ | Backend complete; frontend in `pendingFrontendCommandNames`; merge UI not built (#111) |

**Legend:**
- ✅ Complete — built, tested, and included in the closed release train
- 🔄 Partial — core functionality built; known gap or verification outstanding
- ⏳ Planned — scoped but not yet implemented

---

### Branch / Feature Completion Detail

#### Open Items (not yet complete)

| Item | Journey | Gap ID | Current State | Target Milestone |
|------|---------|--------|--------------|-----------------|
| `publishBatchMedia` does not update `batches.mediaStatus` | J13 (Photography), step 5 | — | Modern upload+publish flow leaves `mediaStatus='open'`; only legacy `attachBatchPhoto` updates it | Patch in next photography pass |
| Contact merge / deduplicate UI | J14 (Contacts) / cross-journey | TODO in `ContactsView.tsx` | `contact_merge_candidates` table exists; detection partially implemented; merge UI not built | Phase 7 or post-launch |
| Frontend wiring for `internalOnlyCommandNames` (8) | Multiple (Connectors, Credit Review) | Issue #111 | Backend handlers exist; UI surfaces missing | Phase 7 |

#### Recently Closed Gaps

| Gap | Status | Evidence |
|-----|--------|----------|
| DYN-H3 / Issue #26 — Payments FIFO auto-allocation | ✅ Closed | `logPayment` auto-executes `allocatePayment` in the same transaction (commandBus.ts:3697-3731), degrades gracefully when no open invoices |
| DYN-H4 / Issue #27 — Matchmaking status lifecycle | ✅ Closed | `assertValidNeedStatusTransition` and `assertValidSupplyStatusTransition` enforced server-side (commandBus.ts:5197-5224) |

#### Phase 7 Items (scoped, not yet started)

| Item | Description | Phase |
|------|-------------|-------|
| Keyboard sweep | All core journeys must be completable keyboard-first | Phase 7 |
| Focus / drawer persistence | Operators can expand/minimize context without losing place | Phase 7 |
| Vocabulary / drift audit | Vocab-clean copy and no drift from design system | Phase 7 |
| Deployment packaging | Self-hosted production build, environment hardening | Phase 7 |

---

### Milestone Map

| Milestone (Phase) | Journeys / Capabilities | Status |
|-------------------|------------------------|--------|
| **Phase 0a** — Foundation | Design system cleanup, CommandPalette, WorkspacePanel | ✅ Complete |
| **Phase 0b** — Canvas Grammar | Keel, SideNav, Identity Ribbon, Context Drawer, focus states | ✅ Complete |
| **Phase 1** — Customer Selling | Sales Order (J4), Product Finder, customer identity ribbon | ✅ Complete |
| **Phase 2** — Procurement & Inventory | Purchase Orders (J1), Intake (J2), Inventory (J3), Photography (J13) | ✅ Complete |
| **Phase 3** — Money Loop | Payments/AR (J6), Vendor Bills/AP (J7), Credit Review (J12) | ✅ Complete |
| **Phase 4** — Sell Follow-Through | Fulfillment (J5), Connectors (J11), Client Ledger | ✅ Complete |
| **Phase 5** — Recovery & Closeout | Recovery (J9), Closeout (J8) | ✅ Complete |
| **Phase 6** — Reports & Dashboard | Reports, Dashboard, source-row drilldowns | ✅ Complete |
| **Phase 7** — Polish & Release | Keyboard sweep, focus persistence, vocab audit, deployment | ⏳ Not started |
| **Post-launch** | Contact merge UI, frontend wiring for internalOnlyCommandNames (#111) | ⏳ Planned |
